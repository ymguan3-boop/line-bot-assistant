...
app.post('/webhook', line.middleware(config), (req, res) => {
    // 立即回應 200 OK 給 LINE 平台，避免 Timeout
    res.status(200).end();
...
async function handleEvent(event) {
    ...
    try {
        ...
        // console.error('無法取得用戶資訊:', err); // 常見錯誤，可忽略或記錄
    }
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            stream.on('end', () => {
                ...
                const filename = `${dateStr}_${safeUserName}_${messageId}.${ext}`;
